...
//从加密锁中读取一批字节
//int YRead(byte[] OutData, short Address, short mylen, string HKey, string LKey, string KeyPath);

//写一批字节到加密锁中
//int YWrite(byte[] InData, short Address, short mylen, string HKey, string LKey, string KeyPath);
...
namespace WindowsApplication1
{
   public struct  GUID
    ...
    class SoftKey2K
    {
        ...
        private static extern bool SetupDiEnumDeviceInterfaces_64(IntPtr DeviceInfoSet, int DeviceInfoData, ref GUID InterfaceClassGuid, int MemberIndex, ref SP_INTERFACE_DEVICE_DATA_64 DeviceInterfaceData);
    ...
    private int NT_FindPort_2(int start, int in_data, int verf_data, ref string OutPath )
{
    ...
    public int NT_GetIDVersion(ref short Version, string Path)
    {
        int ret;
        int hsignal;
        hsignal = CreateSemaphore(0, 1, 1, "ytdgyzt_6d");
